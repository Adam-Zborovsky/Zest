...
void main() {
  ({
    ...
    List<Uri> requests,
    void Function(List<String>) setStocked,
  })
  setup({
    ...
    return (container: container, requests: requests, setStocked: setStocked);
  }

  RecipeScope scope(int count) => RecipeScope(
    label: 'Recipes for “test”',
    recipes: [
      for (var id = 1; id <= count; id++) summary('$id', 'Testbench $id'),
    ],
  );

  test('starting without a scope or selection leaves the run idle', () async {
    final test = setup(
      respond: (_) async => lookupResponse('1'),
      stocked: const [],
    );
  ...
  test(
    'a full run checks every recipe and classifies the three buckets',
    () async {
      final test = setup(
  ...
  test(
    'lookups that miss still count as checked and reported unavailable',
    () async {
      final test = setup(
        respond: (request) async => request.url.queryParameters['i'] == '3'
  ...
  test(
    'a rate limit pauses the run and keeps partial matches for resume',
    () async {
  ...
  test('changing the selection or scope resets a finished run', () async {
    ...
    test.setStocked(['Imaginary gin', 'Pretend lime juice']);
    expect(test.container.read(barMatchProvider).status, BarMatchStatus.idle);
