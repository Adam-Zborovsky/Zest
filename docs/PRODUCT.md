...
- Search is resolved for M11 (2026-09-14): name, ingredient and letter results in Discover, and every suggestion surface, read the local shared-catalog search index instead of a per-query provider call. Coverage is exactly what the downloaded snapshot holds, stated the same way as other catalog-wide counts — never a completeness claim.
