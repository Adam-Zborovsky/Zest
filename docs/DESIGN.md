...
- **Constellation.** The canvas sits unboxed on the night field over a star-dot texture (`NightDotField`), which is kept out from behind text so rendered contrast checks stay meaningful. Nodes are cut-paper glyph discs grouped by ingredient name: spirits are moss with a tall bottle, liqueurs and bitters berry with a round corked flask, citrus grapefruit with a wheel, sweeteners peach with an isometric sugar cube, herbs and spice celery with a mint sprig, and everything else a quiet disc with a tumbler and bubbles. Home opens with nothing selected, and a selection does not survive leaving home. Lines are peach string, the selected neighborhood is celery, and the selected edge and node halo are grapefruit. Labels are peach pills. A legend under the search field names only the groups present and states "Colors group ingredients by name, not by flavor."
...
- **Drink calendar (2026-09-13).** The collection is a month calendar in the spirit of Instagram's archived stories. A day with one drink is a rounded photo tile; a day with several shows floating circles, the smaller ones tucked slightly under the largest like stacked stickers. The white date number sits on the photo or on the largest circle over a 45% leaf tint with a soft shadow, which keeps it legible over any picture. Today gets a grapefruit ring, empty days show a quiet secondary-ink number, and future days use disabled ink. A memory photo wins; otherwise the cocktail's small source image; otherwise the glass artwork. See [COLLECTION.md](COLLECTION.md).
