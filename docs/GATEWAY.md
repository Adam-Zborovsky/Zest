...
- **Refresh**: a background job walks `search.php?f=` for all 26 letters through this same gateway (so it shares the 429 cooldown above) on startup (if there is no published snapshot, the last check failed, or the last check was over 24 hours ago) and every 24 hours after a successful or unchanged check. It never blocks server startup.
- **Retry on failure**: a single self-rescheduling timer, not a fixed interval, so a failed run does not wait out the rest of the 24-hour interval. It retries with bounded exponential backoff — 5 minutes, doubling each further consecutive failure, capped at the 24-hour interval — and honors a gateway `Retry-After` (e.g. from a 429 cooldown) when it asks for longer than the computed backoff. A success or unchanged check resets the backoff and reschedules the full 24 hours out.
...
- **Response**: `200` with a quoted strong `ETag` and `Cache-Control: no-cache` (this route only; every other route keeps the process-wide `no-store`), `304` on an `If-None-Match` that lists the current version (a comma-separated list, weak `W/"..."` validators, and `*` all match, per RFC 9110's weak comparison for this header), `503 catalog_unavailable` before the first publish, and gzip when `Accept-Encoding` includes it. State and drinks are read as one consistent pair, so a served body never mixes one version's `version` with another's `drinks`.
